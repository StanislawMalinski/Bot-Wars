          ...
          <div>
            ...
            <SeedTeam style={{ color: getColor(seed, index) }}>
                <div className='ladder-record'>
                    <div className='user-name' style={{ color: getColor(seed,index)}}>
                        {team.UserName|| '...'}
                    </div>
                    <div className='bot'>
                        {getImage(seed, index)}
...
const getEmptySeed = function (id) {
    return {
        id: id,
...
const buildRounds = function (json) {
    ...
    for (let i = 0; i < minPosition; i++) {
        temp.push(getEmptySeed(i));
    };
    temp = temp.concat(result);
    const tail = heapSize - temp.length;
    for (let i = 0; i < tail; i++) {
        temp.push(getEmptySeed(i + maxPosition + 1));
    }
    result = temp;
    ...
    result = result.map(element => {
        ...
        if (!element.Position) element.Position = element.id
        return element;
    });
